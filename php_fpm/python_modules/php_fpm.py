...
class FCGIApp(flup.client.fcgi_app.FCGIApp):
    ...
    ### workaround for a php-fpm bug: http://bugs.php.net/bug.php?id=53618
...
class UpdatePhpFpmThread(threading.Thread):
    ...
    def _get_php_fpm_status_response(status_path, host, port):
        ...
        try:
            ...
            logging.error('Could not deserialize json: ' + result)
            raise Exception('Could not deserialize json: ' + result)
...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_option('-H', '--host', dest='host', default='localhost', help='PHP-FPM host')
        parser.add_option('-P', '--ports', dest='ports', default='9000', help='PHP-FPM port')
